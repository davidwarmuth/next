...
export default function TasksDashboardPage() {
  ...
  return (
    <main className="px-4 py-6 w-full container sm:mx-auto min-h-[calc(100svh-64px)]">
      ...
      <div className="py-20 flex gap-8 justify-around">
        <Button asChild variant="ghost" className="h-fit flex-col">
          <Link href="/tasks/list">
            <List className="!size-40 opacity-20" />
            <span className="-mt-4 pb-4 text-muted-foreground text-xl">
              Go to list of tasks
            </span>
          </Link>
        </Button>
        <div className="border-r-8 rounded-md opacity-60"></div>
        <Button asChild variant="ghost" className="h-fit flex-col">
          <Link href="/tasks/new">
            <ListPlus className="!size-40 opacity-20" />
            <span className="-mt-4 pb-4 text-muted-foreground text-xl">
              Create new task
            </span>
          </Link>
